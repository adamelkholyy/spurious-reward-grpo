...
The output JSON (--out, default results_output_dist.json) is a list of
...
Minimal plotting example:
    ...
    results = json.load(open("results_output_dist.json"))
    for r in results:
...
def build_prompts(tokenizer, num_prompts: int, seed: int):
    ...
    Uses a seed-local RNG so every model sees the *same* questions
...
def main():
    ap = argparse.ArgumentParser(
        description="Measure output distributions of models on GSM8K "
                    "rollout prompts.")
