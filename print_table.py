...
def print_gsm8k_table(results_path="results/results_gsm8k.json",
    ...
    latex:          emit a LaTeX table (booktabs) instead of the ASCII one.
    sort_by_model:  group each run under its baseline family instead of
                    listing all baselines first and then all runs.
    skip_substrings: labels containing any of these (case-insensitive) are
                    dropped. Pass None/() to keep everything.
    """
            ...
            for key, r in json.load(f).items():
                if key not in seen:
                    rows.append({**r, "eval_config": key,
                                 "is_baseline": True, "_from_cache": True})
    ...
    for r in rows:
        ...
        # Results-file rows win over cache rows regardless of merge order.
    ...
    for (n, temp, n_prob, max_tok), by_label in sorted(
        ...
        if sort_by_model:
            ...
            for b in BASELINE_FAMILIES:
                ...
                sec += sorted((r for r in run_results
                               if family(r.get("label")) == b), key=by_label_sort)
                if sec:
                    sections.append(sec)
            orphans = sorted((r for r in run_results
                              if family(r.get("label")) is None),
                             key=by_label_sort)
            if orphans:
                sections.append(orphans)
        else:
            sections = [[base_results[b] for b in BASELINE_FAMILIES
        ...
        def deltas(r):
            ...
            if not b:
                return "--", "--"
            return (f"{(r['accuracy'] - b['accuracy']) * 100:+.1f}pp",
        ...
        caption = (f"GSM8K results   (n={n}, T={temp}, {n_prob} problems, "
...
def _print_latex(sections, cells, k_cols, n, caption):
    ...
    print(r"\toprule")
    print(" & ".join(_tex(h) if not h.startswith("$") else h
                     for h in head) + r" \\")
    print(r"\midrule")
    for i, sec in enumerate(sections):
        if i:
...
if __name__ == "__main__":
    ...
    p.add_argument("results_path", nargs="?",
                   default="results/results_gsm8k.json")
    p.add_argument("--baseline-cache", default=None)
    p.add_argument("--latex", action="store_true",
                   help="emit a LaTeX (booktabs) table")
    p.add_argument("--sort-by-model", action="store_true",
